...
A step's output is what the command printed; `self.note("...", *marks)` publishes, beside the step that preceded it, what the test made of that output -- the element it found, where, and what that proves -- with the pieces of the output it names given as marks so the page can point at them. Notes are normalised like output, and a note outside a documented demo is not recorded.
...
Each demo also publishes its terminal session beside its clip, as an asciicast v2 file the website plays on the clip's timeline: the command line where the demo ran it, its output where the command finished, and an exit line. It is written from the trace rather than captured from a terminal, so it carries the same normalised output the transcript does, lasts as long as the clip, and comes out byte-identical from two generations of one run.
