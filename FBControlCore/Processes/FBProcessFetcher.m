...
@property (nonatomic, readonly, assign) size_t argumentBufferSize;
@property (nonatomic, readonly, assign) char *argumentBuffer;

@property (nonatomic, readonly, assign) size_t pidBufferSize;
@property (nonatomic, readonly, assign) pid_t *pidBuffer;
...
- (NSArray<FBProcessInfo *> *)processesWithProcessName:(NSString *)processName
{
  ...
  IterateAllProcesses(self.pidBuffer,
    self.pidBufferSize, ^BOOL (pid_t pid) {
...
- (pid_t)processWithOpenFileTo:(const char *)filename
{
  ...
  IterateOpenFiles(self.pidBuffer,
    self.pidBufferSize,
    filename, ^BOOL (pid_t pid) {
...
+ (FBFuture<NSString *> *)performSampleStackshotForProcessIdentifier:(pid_t)processIdentifier queue:(dispatch_queue_t)queue
{
  return [[[[[FBProcessBuilder
              withLaunchPath:@"/usr/bin/sample"
              arguments:@[@(processIdentifier).stringValue, @(SampleDuration).stringValue]]
