...
#pragma mark ProcessFileOutput

@interface FBProcessFileOutput_DirectToFile : NSObject <ProcessFileOutput>
...
- (FBFuture<NSNull *> *)startReading
{
  return [[[[[FBFuture
           ...
           onQueue:self.queue
           map:^NSNull *(id<ProcessFileOutput> nested) {
             self.nested = nested;
             return NSNull.null;
           }]
          named:[NSString stringWithFormat:@"Start Reading %@", self.description]];
...
@property (nonatomic, readonly, strong) FBMutableFuture<NSNumber *> *readFuture;
@property (nonatomic, readonly, strong) NSInputStream_FBProcessOutput *stream;

@end

@interface NSInputStream_FBProcessOutput : NSInputStream

@property (nonatomic, readonly, strong) FBFuture<NSNumber *> *readFuture;
@property (nonatomic, readwrite, assign) int fileDescriptor;
...
@property (nonatomic, readonly, strong) id<AccumulatingBuffer> dataConsumer;
...
- (FBFuture<id<ProcessFileOutput>> *)providedThroughFile
{
  return [[self
           makeFifoOutput]
          onQueue:self.workQueue
          map:^(NSString *fifoPath) {
...
- (FBFuture<FBProcessStreamAttachment *> *)attach
{
  return [[super
           attach]
          onQueue:self.workQueue
          map:^(FBProcessStreamAttachment *result) {
...
- (FBFuture<FBProcessStreamAttachment *> *)attach
{
  return [[super
          ...
          fmap:^(FBProcessStreamAttachment *attachment) {
            ...
            self.reader = [FileReader readerWithFileDescriptor:self.readEnd closeOnEndOfFile:YES consumer:consumer logger:self.logger];
            return [[[self.reader
                      startReading]
...
- (FBFuture<id<ProcessFileOutput>> *)providedThroughFile
{
  return [[[self
            makeFifoOutput]
           onQueue:self.workQueue
           map:^id<ProcessFileOutput>(NSString *fifoPath) {
...
- (instancetype)initWithMutableData:(NSMutableData *)mutableData
{
  id<AccumulatingBuffer> consumer = [FBDataBuffer accumulatingBufferForMutableData:mutableData];
