...
public enum AXKeys: String, Sendable, CaseIterable {
  case label = "AXLabel"
  case frame = "AXFrame"
  case value = "AXValue"
  ...
  public static let occluderIdentityKeys: Set<AXKeys> = [.interactable, .frameDict, .type, .uniqueID, .label]

  /// The keys the application answers by hit-testing every node, which is what makes a read expensive.
  public static let reachabilityKeys: Set<AXKeys> = [.interactable, .occludedBy]

  /// Every key, including the costly `interactable` and `occludedBy`.
  public static let everything: Set<AXKeys> = Set(allCases)
  ...
  public static func acceptsArgument(_ raw: String) -> Bool {
    raw == everythingToken || AXKeys(rawValue: raw) != nil
  }

  /// The keys a caller asked for, expanding `all` and dropping anything unrecognised. `all` wins over
  /// any keys listed alongside it.
  public static func requested(_ raw: [String]) -> Set<AXKeys> {
    raw.contains(everythingToken) ? everything : Set(raw.compactMap(AXKeys.init(rawValue:)))
  }

  /// Default set of keys returned when no specific keys are requested.
  public static let defaultSet: Set<AXKeys> = [
...
/// Subset of `AXKeys` whose values are strings, suitable for element search matching.
public enum AXSearchableKey: String, Sendable {
  case label = "AXLabel"
  case uniqueID = "AXUniqueId"
  case value = "AXValue"
  case title = "title"
  ...
  /// The `AXKeys` whose serialized field a marker match reads. A marker is matched over the
  ...
  /// requested key set — notably `.placeholder`, which `AXKeys.defaultSet` omits.
  public var serializationKey: AXKeys {
    switch self {
...
public enum AccessibilityScrollDirection: Sendable {
  case up
  case down
  case left
  case right
