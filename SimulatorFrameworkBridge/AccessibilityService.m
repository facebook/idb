...
)
{
        ...
        *truncated = YES;
        ...
        if (continuation) {
          return FBAXBridgeNodeFromSnapshot(
            runtime, continuation, fetchList, continuationNames, elementPid, depth, maxDepth, budget,
            boundaryFetches, truncated);
        }
  ...
  for (id child in nesting) {
    NSDictionary *built = FBAXBridgeNodeFromSnapshot(
      runtime, child, fetchList, namesByNumber, ownerPid, depth + 1, maxDepth, budget, boundaryFetches,
      truncated);
    if (built) {
      [children addObject:built];
    }
...
static NSDictionary<NSString *, id> *FBAXBridgeDispatchRequest(NSDictionary<NSString *, id> *request)
{
  ...
  if ([request[kRequestSnapshotTree] boolValue]) {
    ...
    pid_t ownerPid = [snapshot isKindOfClass:NSDictionary.class]
      ? [runtime owningProcessIdentifierForSnapshotElement:((NSDictionary *)snapshot)[kSnapshotElement]]
      : 0;
    int boundaryFetches = kSnapshotBoundaryFetchBudget;
    tree = FBAXBridgeNodeFromSnapshot(
      runtime, snapshot, names, namesByNumber, ownerPid, 0, maxDepth, &budget, &boundaryFetches,
      &truncated);
    if (!tree) {
