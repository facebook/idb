...
@Suite struct XcodeConfigurationTests {

  private func writePlist(_ contents: [String: Any]) throws -> String {
    let path = (NSTemporaryDirectory() as NSString).appendingPathComponent("XcodeConfigurationTests-\(UUID().uuidString).plist")
  ...
  @Test func readsStringValueForPresentKey() throws {
    ...
    let value = XcodeConfiguration.readValue(forKey: "Version", fromPlistAtPath: path)
  ...
  @Test func readsNonStringValueForPresentKey() throws {
    ...
    let value = XcodeConfiguration.readValue(forKey: "Count", fromPlistAtPath: path)
    #expect((value as? NSNumber)?.intValue == 3)
  }

  @Test func returnsNilForMissingPlist() {
    let path = (NSTemporaryDirectory() as NSString).appendingPathComponent("XcodeConfigurationTests-missing-\(UUID().uuidString).plist")
    let value = XcodeConfiguration.readValue(forKey: "Version", fromPlistAtPath: path)
    #expect(value == nil)
  }
  ...
  @Test func returnsNilForRelativePlistPathFromEmptyDeveloperDirectory() {
    ...
    let value = XcodeConfiguration.readValue(forKey: "Version", fromPlistAtPath: "Platforms/iPhoneSimulator.platform/Info.plist")
    #expect(value == nil)
  }

  @Test func returnsNilForUnreadablePlist() throws {
    let path = (NSTemporaryDirectory() as NSString).appendingPathComponent("XcodeConfigurationTests-garbage-\(UUID().uuidString).plist")
  ...
  @Test func returnsNilForMissingKey() throws {
    let path = try writePlist(["Version": "17.4"])
    let value = XcodeConfiguration.readValue(forKey: "SomeOtherKey", fromPlistAtPath: path)
    #expect(value == nil)
  }
}
