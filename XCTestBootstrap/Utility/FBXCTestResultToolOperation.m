...
+ (FBFuture<FBIDBProcess *> *)internalOperationWithArguments:(NSArray<NSString *> *)arguments queue:(dispatch_queue_t)queue logger:(nullable id<FBControlCoreLogger>)logger
{
  ...
  return [[[[[[FBIDBProcessBuilder
    ...
    onQueue:queue map:^(FBIDBProcess *task) {
      return task;
    }];
}

+ (FBFuture<FBIDBProcess *> *)exportFrom:(NSString *)path to:(NSString *)destination forId:(NSString *)bundleObjectId withType:(NSString *)exportType queue:(dispatch_queue_t)queue logger:(nullable id<FBControlCoreLogger>)logger
{
...
+ (NSDictionary<NSString *, NSDictionary<NSString *, id> *> *)getJSONFromTask:(FBIDBProcess *)task
{
