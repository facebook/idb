...
+ (FBFuture<NSNull *> *)parse:(NSString *)resultBundlePath target:(id<FBiOSTarget>)target reporter:(id<FBXCTestReporter>)reporter logger:(id<FBControlCoreLogger>)logger extractScreenshots:(BOOL)extractScreenshots
{
  ...
  if (results) {
    ...
    return [[FBXCTestResultToolOperation
            ...
            fmap:^(NSDictionary<NSString *, NSDictionary<NSString *, id> *> *actionsInvocationRecord) {
...
+ (void)reportTestMethod:(NSDictionary<NSString *, NSObject *> *)testMethod testBundleName:(NSString *)testBundleName testClassName:(NSString *)testClassName reporter:(id<FBXCTestReporter>)reporter
{
  ...
  if (status == FBTestReportStatusFailed) {
    ...
    [reporter testCaseDidFailForTestClass:testClassName
     ...
     extractScreenshots:(BOOL)extractScreenshots
{
  ...
  if (testMethods != nil) {
    [self reportTestMethods:testMethods
             testBundleName:testBundleName
              testClassName:testClassName
                   reporter:reporter
                      queue:queue
       ...
       extractScreenshots:(BOOL)extractScreenshots
{
  ...
  for (NSDictionary *testMethod in testMethods) {
    [self reportTestMethod:testMethod
            testBundleName:testBundleName
             testClassName:testClassName
                  reporter:reporter
                     queue:queue
      ...
      extractScreenshots:(BOOL)extractScreenshots
{
  ...
  if (summaryRefId != nil) {
    [[[FBXCTestResultToolOperation
       ...
       logger:logger]
      onQueue:queue
      doOnResolved:^(NSDictionary<NSString *, NSDictionary<NSString *, id> *> *actionTestSummary) {
        if (status == FBTestReportStatusFailed) {
          NSArray *failureSummaries = accessAndUnwrapValues(actionTestSummary, @"failureSummaries", logger);
          [reporter testCaseDidFailForTestClass:testClassName
                                         method:testMethodIdentifier
                                     exceptions:@[
             [[FBExceptionInfo alloc]initWithMessage:[self buildErrorMessage:failureSummaries logger:logger]]
           ]];
        }
      ...
      }] awaitWithTimeout:XCTestOperationTimeoutSecs
                    error:nil];
  }
}
  ...
  for (NSDictionary<NSString *, NSDictionary *> *attachment in attachments) {
    if (attachment[@"filename"]) {
      ...
      if (attachment[@"payloadRef"] && matchResult) {
        ...
        [[FBXCTestResultToolOperation
          exportJPEGFrom:resultBundlePath
          to:exportPath
          forId:screenshotId
          type:screenshotType
          queue:queue
