...
@property (nonatomic, readonly, strong) FBListTestConfiguration *configuration;
...
@property (nonatomic, readonly, strong) FBListTestStrategy *strategy;
...
- (FBFuture<NSNull *> *)execute
{
  ...
  return [[self.strategy
           listTests]
          onQueue:self.strategy.target.workQueue
          map:^(NSArray<NSString *> *testNames) {
          ...
          pop:^FBFuture *(NSURL *temporaryDirectory) {
            return [[FBOToolDynamicLibs
                    ...
                    fmap:^FBFuture<NSNull *> *(NSArray<NSString *> *libraries) {
                      NSDictionary<NSString *, NSString *> *environment = [FBListTestStrategy setupEnvironmentWithDylibs:libraries
                                                                                                                shimPath:shimPath
                                                                                                      shimOutputFilePath:shimOutput.filePath
...
{
  return [[target launchProcess:spawnConfiguration] onQueue:target.workQueue
                                                        map:^id _Nonnull (FBSubprocess * _Nonnull process) {
