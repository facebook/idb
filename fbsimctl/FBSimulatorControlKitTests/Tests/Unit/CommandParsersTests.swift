...
let validActions: [([String], Action)] = [
  ...
  (["shutdown"], .shutdown),
  (["shutdown"], .shutdown),
  (["stream", "-"], Action.stream(FBVideoStreamConfiguration(encoding: .BGRA, framesPerSecond: nil), .standardOut)),
  (["stream", "/tmp/video.dump"], Action.stream(FBVideoStreamConfiguration(encoding: .BGRA, framesPerSecond: nil), .path("/tmp/video.dump"))),
  (["stream", "--bgra", "-"], Action.stream(FBVideoStreamConfiguration(encoding: .BGRA, framesPerSecond: nil), .standardOut)),
  (["stream", "--h264", "-"], Action.stream(FBVideoStreamConfiguration(encoding: .H264, framesPerSecond: nil), .standardOut)),
  (["stream", "--fps=30", "-"], Action.stream(FBVideoStreamConfiguration(encoding: .BGRA, framesPerSecond: 30), .standardOut)),
  (["stream", "--bgra", "--fps=25", "-"], Action.stream(FBVideoStreamConfiguration(encoding: .BGRA, framesPerSecond: 25), .standardOut)),
  (["stream", "--fps", "60", "/tmp/video.dump"], Action.stream(FBVideoStreamConfiguration(encoding: .BGRA, framesPerSecond: 60), .path("/tmp/video.dump"))),
