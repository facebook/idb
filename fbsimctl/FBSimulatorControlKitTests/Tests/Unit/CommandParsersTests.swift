...
let validActions: [([String], Action)] = [
  ...
  (["shutdown"], .shutdown),
  (["shutdown"], .shutdown),
  (["stream", "-"], Action.stream(.standardOut, .BGRA)),
  (["stream", "/tmp/video.dump"], Action.stream(.path("/tmp/video.dump"), .BGRA)),
  (["stream", "-", "--h264"], Action.stream(.standardOut, .H264)),
  (["stream", "-", "--bgra"], Action.stream(.standardOut, .BGRA)),
