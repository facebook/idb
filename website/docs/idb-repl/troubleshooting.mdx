...
A submission that fails still **exits 0** — nonzero exits are reserved for argument and connection errors. Do not branch on the exit code for your code's success. Instead, check the **prefix** of the output:
...
Paths returned by `IDB.screenshot` / `IDB.video` are on the filesystem of the **machine running the companion**, not necessarily your local machine — relevant when using a remote `--companion`. To collect artifacts locally, run with `--report-path`, which gathers captured files along with the report. See [Reports and replay](reports-and-replay.mdx).
