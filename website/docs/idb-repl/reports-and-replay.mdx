...
`idb-repl` can record a session to a Markdown **report** and later **replay** it. Together these let you capture an exploration or automation run, share it as a readable document, and re-run it later.
...
Artifacts are only kept when running with `--report-path`. (Calls such as `IDB.screenshot.capture()` that also return a path still write that file on the companion regardless; see [Built-in IDB API → Behaviour](built-in-idb-api.mdx#behaviour).)
