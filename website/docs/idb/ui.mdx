...
For repeated reads, `--api axbridge-persistent` keeps the reader warm between them — the companion owns it, so this holds across separate `idb` invocations too; see [Accessibility](accessibility.mdx).
