...
- **HID operations** synthesize the events a physical finger, keyboard or button would produce. They know nothing about elements — a swipe is a swipe whether or not anything is under it.
...
Describes the single element matching a marker, without you having to know where it is.
...
|--------|-----------|-------------|
| `--api {ax,axbridge,axbridge-persistent}` | all three | Which backend serves the read. See [Accessibility](accessibility.mdx) |
...
These commands resolve a target through the accessibility system and then perform the action the platform itself would perform. They are not synthesized touches: an accessibility press activates the element directly, so it is unaffected by an animation moving the element mid-gesture, and it cannot land on the wrong thing because something moved under the coordinate.

### `idb ui tap`

`tap` is the one command that spans both worlds, and which world it lands in depends on how you address it:
...
`--duration` holds the touch down for a given time, and is only meaningful for a coordinate touch. An accessibility press is instantaneous with nowhere to put a hold, so `--duration` is rejected there rather than being dropped — a long press that quietly became a tap would make a test pass for the wrong reason.
...
The read and act commands are designed to compose. A read tells you what is on screen and a marker names an element in it, so an interaction never has to hardcode a coordinate that a different device size or a shifted layout would invalidate:
...
For repeated reads in a long-running process, `--api axbridge-persistent` keeps the reader warm between them; see [Accessibility](accessibility.mdx).
