...
`idb rotation` reads the simulator's current physical orientation. Add `--json`
for `{"orientation": "PORTRAIT"}`. Reads also preserve `UNKNOWN`, `FACE_UP`, and
`FACE_DOWN` when the runtime reports them.

`idb rotation PORTRAIT` sets physical orientation. Supported values are
`PORTRAIT`, `PORTRAIT_UPSIDE_DOWN`, `LANDSCAPE_LEFT`, and `LANDSCAPE_RIGHT`.
The getter uses the same direction names as the setter and observes changes
made in Device Hub. An app may keep its interface orientation locked even when
the physical device rotates.

The existing `idb ui rotate` command retains its historical behavior. On legacy
simulators its landscape directions use the opposite convention; use the new
`rotation` commands together for physical orientation.

## Simulator hinge

`idb hinge` reads a fresh measured hinge angle in degrees. `idb hinge ANGLE` sets the angle from 0 (closed) to 180 (flat). These commands require a simulator runtime that supports hinge motion. They are validated with iPhone Duo on iOS 27.1 and Xcode 27.1.
...
Device Hub's Closed, Book and Open presets correspond to 0, 130 and 180 degrees. Reads also observe changes made in Device Hub; during an animation they can report an intermediate angle. `get --json` prints an object such as `{"angle": 130.0}`. A read fails if the simulator cannot provide a fresh valid sample within five seconds.
...
idb ui key 4
# Wait up to 30 seconds for the UI to settle
idb ui quiet 30
```
...
```
idb notification send com.apple.Maps '{"aps": {"alert": "Hello"}}'
```

Delivers a push notification payload to an installed app, with no APNs certificate or network setup. The payload is the JSON the app would receive from APNs. The original `idb send-notification` spelling remains supported for existing callers.

### List delivered notifications

```
idb notification list com.apple.Maps
# One JSON object per notification, one per line
idb notification list --json com.apple.Maps
```

Lists the notifications an app has had delivered to it and still holds on a simulator. Physical devices are not supported. The app does not have to be running, and nothing here launches or foregrounds it, so this can be read after a test has killed the app it pushed to.

Each notification is reported with its bundle ID, identifier, title, subtitle, body, thread identifier, and delivery date. Without `--json` the text fields are quoted and escaped as JSON strings, so a title or body carrying a newline still reads as one record on one line; text outside ASCII is left as the app wrote it rather than escaped, and a field the app left empty reads as `""`. A ` | ` inside a field survives the quoting, so the quotes show where a field ends rather than making a naive split on ` | ` safe — use `--json` where fields have to be taken apart reliably. The date is seconds since the Unix epoch under `--json`, and an ISO 8601 UTC timestamp otherwise. A notification that carried no date reads as `null` and `no date` respectively, rather than being given an epoch timestamp. A finite date too far out for the host calendar remains numeric under `--json` and reads as `unreadable date` in the text form.

### Clear delivered notifications

```
idb notification clear com.apple.Maps
```

Withdraws every notification an app has had delivered to it on a simulator, so they leave Notification Center and a later `idb notification list` reports none. Physical devices are not supported. As with listing, the app does not have to be running and nothing here launches or foregrounds it, which lets a test start from an empty inbox rather than whatever earlier runs left behind.
...
The `idb_companion` is a native executable, written in Swift, that is used to manipulate iOS Simulators & Devices. It implements many of the CRUD-style operations associated with iOS resources and exposes a `gRPC` interface that can be consumed by the `idb` client.
...
### Managing targets from the companion

The companion also exposes target management directly, as one-shot flags instead of a server. These are the operations that are deliberately kept off the `gRPC` interface because they are destructive, or because they concern targets that do not have a server running yet:

```
idb_companion --list 1                  # List available targets
idb_companion --boot UDID               # Boot a simulator
idb_companion --reboot UDID             # Reboot a target
idb_companion --shutdown UDID           # Shut a target down
idb_companion --create "DEVICE"         # Choose the newest available compatible runtime
idb_companion --create "DEVICE,RUNTIME" # Require a particular runtime
idb_companion --clone UDID              # Clone a simulator
idb_companion --erase UDID              # Erase a target's contents and settings
idb_companion --delete UDID|all         # Delete a simulator, or all of them
idb_companion --recover ecid:ECID       # Enter recovery mode (devices)
idb_companion --unrecover ecid:ECID     # Exit recovery mode (devices)
idb_companion --activate ecid:ECID      # Activate a device
```

For creation, replace `DEVICE` with an installed device type's display name or
`com.apple.CoreSimulator.SimDeviceType.*` identifier. Optionally append a comma
and `RUNTIME`, using its display name or `com.apple.CoreSimulator.SimRuntime.*`
identifier. Quote the argument when names contain spaces. List the device types
and runtimes for the selected Xcode with `xcrun simctl list devicetypes` and
`xcrun simctl list runtimes`.

Omitting `RUNTIME` selects the newest available runtime compatible with the device.
An explicit runtime must match and be available and compatible; creation fails
instead of falling back to another runtime. CoreSimulator determines the build
used by the created device, including when multiple installed builds share the
same runtime identifier. A runtime identifier does not pin a particular build.

The full list is in `idb_companion --help`.
