...
The `idb ui` command group reads the screen through the target's accessibility system, acts on the elements it reports, and synthesizes HID events. It has a page of its own: [UI Automation](ui.mdx) documents the whole group, and [Accessibility](accessibility.mdx) covers the backends that serve a read and the formats a read comes back in.
