...
A instance of a target (`Target`) is an object that represents a single iOS Simulator or Device. `Target` is a protocol definition that describes the functionality that is implemented by both `Simulator` and `Device`. This abstraction means that higher-level applications and Frameworks are able to treat a target the same, regardless of whether it is an iOS Simulator or Device.
...
A "Target Set" (`TargetSet`) represents a collection of targets. These are implemented in both `SimulatorSet` and `DeviceSet`. A Simulator set represents a root directory that is common to a number of Simulators. A Device Set represents all of the Devices attached to the host.
...
Across the Frameworks, there are "Configuration" values. These are typically used for consolidating all the information required for a particular API call. For instance `ApplicationLaunchConfiguration` defines launch arguments, environment and launch modes.
...
There may be some capabilities that are only supported by one or the other, depending on the target. For instance there is no concept of "Activation" on an iOS Simulator, so `activate()` is only declared on `Device`. Where a target does not support a given protocol, `idb` will fail when these APIs are called, giving an error message that explains what functionality is missing.

Implementations across Simulators & Devices are completely separated and implemented in their respective Frameworks. As an example, `ApplicationCommands` (which provides an API for launching and listing Applications on an iOS Target), is implemented separately in `SimulatorApplicationCommands` and `DeviceApplicationCommands`.

If functionality is common to both Simulators & Devices, its protocol is added to `Target` so that implementors of `Target` are required to implement it. For functionality that is not common, the relevant protocol is added to the definition of the concrete `Simulator` or `Device` class. For non-common protocols, the caller must either check for protocol conformance before calling an API, or use the concrete type directly.
...
Alongside it is a "structured logger" (`EventReporter`), which receives typed events rather than arbitrary strings. The `idb_companion` wraps every gRPC call in `CompanionTelemetry`, which reports one event per call, so the server produces accurate logging of all API calls that are made against it. `EventReporter` is a protocol, so a consumer can supply its own implementation and push these events into a datastore that supports aggregation.

### IO

Due to the nature of the functionality that is offered in the Frameworks, IO is a very common task. There are a number of abstractions for reading and writing data between various sources and sinks. For example, the common interface (`FileReader`) is used to read output from a spawned Application process and relay it to a consumer (`FileWriter`). This is then used to pipe Application output over `idb`'s gRPC interface without the Application launcher having to be aware of what is consuming the output.
...
### [`GRPCSwiftServer`](https://github.com/facebook/idb/blob/main/CompanionLib/SwiftServer/GRPCSwiftServer.swift)
...
### [Method handlers](https://github.com/facebook/idb/tree/main/CompanionLib/SwiftServer/MethodHandlers)
...
### [`IDBCommandExecutor`](https://github.com/facebook/idb/blob/main/CompanionLib/IDBCommandExecutor.swift)
