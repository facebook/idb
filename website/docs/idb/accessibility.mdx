...
`--filter interactable` does not put a read on that path. The filter matches on label, identifier and role, none of which are hit-tested, so a filtered read keeps the one-call fetch.
