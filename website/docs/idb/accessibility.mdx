...
| Field | What it measures |
|-------|------------------|
| `traversal` | The traversal that produced this read — `view-hierarchy`, `semantic` or `single-fetch`. Read `mach_round_trips` against it: the same tree costs one round trip per process or one per node depending on the traversal. Which traversal a read gets is described in [How the axbridge backends fetch the tree](#how-the-axbridge-backends-fetch-the-tree) |
