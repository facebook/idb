...
If you wish to make a recording for a target you can use `idb video FILE_PATH`.
...
Video streaming allows for live frames to be captured from the iOS Target. Typically, this stream should be piped through another application for consumption, depending on the use-case. `idb` provides raw access to the video, in a variety of encodings, it's left to the user to decide how to combine this with a downstream video streaming pipeline. Streaming video pairs well with the [UI Automation commands](ui.mdx).
...
For example, the following invocation can be used to start a video stream and expose it on a UDP port.
...
- `--fps 30` is the number of frames that are produced by `idb` per second. This can be arbitrarily large or small. A higher frame rate will increase system utilization. Increasing the fps may not result in smoother presentation, as an iOS Simulator may be refreshing its screen less frequently than the target frame rate. Typically an iOS Simulator may not render transparencies at 60fps.
- `--format h264` represents the format of the video stream itself. A variety of outputs are available:
  - `h264` This is an [Annex-B H.264 Stream](https://en.wikipedia.org/wiki/Network_Abstraction_Layer#NAL_Units_in_Byte-Stream_Format_Use)
  - `rbga` is a stream of raw RGBA bytes. (The format's name preserves a historical misspelling — the flag value really is `rbga`.)
  - `mjpeg` is a stream of encoded JPEG images, [typically called MJPEG](https://en.wikipedia.org/wiki/Motion_JPEG).
  - `minicap` is [the format used by the minicap project](https://github.com/DeviceFarmer/minicap). It's fundamentally an MJPEG stream with a header at the start of the stream and length [headers per frame](https://github.com/DeviceFarmer/minicap#global-header-binary-format).
...
- `--scale-factor` scales the source video down before it is encoded, as a value between 0.0 and 1.0. Reducing it is the cheapest way to cut the bandwidth of a stream, at the cost of resolution.
- `idb video-stream` takes a positional argument for a file path to stream to. When this is not provided, video will be streamed to `stdout`.
