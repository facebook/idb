...
|--------|-------------|------------|---------|
| `--application` | The container of all installed "Application Sandboxes". These are bundle ids corresponding to installed Application home directories | ✅ | ✅ (Only for Applications signed with a Developer Profile) |
...
```
idb file push --application src1.jpg src2.jpg com.foo.bar/Documents
```

Copies one or more files from the host to the relative destination path within the container. Here, two images from the current directory on the host are copied into the `Documents` directory of the app's sandbox.
...
```
idb file mv --application com.foo.bar/src1.jpg com.foo.bar/src2.jpg com.foo.bar/Documents
```

Moves one or more files to a destination path, entirely within the container. Here, two images at the root of the app's sandbox move into its `Documents` directory.
