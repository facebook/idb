...
This mediated connection between the injected application process and IDE Host also allows for [test results to be delivered across a delegate protocol to the IDE Host](https://github.com/facebook/FBSimulatorControl/blob/main/PrivateHeaders/XCTest/XCTestManager_IDEInterface-Protocol.h).
...
The default installation instructions for idb will create shims for you. If you're re-locating the `idb_companion` you should take a [look at this class](https://github.com/facebook/idb/blob/main/XCTestBootstrap/Configuration/FBXCTestShimConfiguration.m), which describes how idb finds these binaries.
