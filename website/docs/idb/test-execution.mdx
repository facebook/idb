...
There are four areas that idb aims to solve on top of Xcode and `xcodebuild`:
...
This mediated connection between the injected application process and IDE Host also allows for [test results to be delivered across a delegate protocol to the IDE Host](https://github.com/facebook/idb/blob/main/PrivateHeaders/XCTestPrivate/XCTestManager_IDEInterface-Protocol.h).

## idb and Shims

In order for idb to extract structured output from a test process, it uses a "shim": a dynamically linked library that can interpose some of the functionality of `XCTest.framework`. The shim sources live under [`Shims/Shimulator`](https://github.com/facebook/idb/tree/main/Shims/Shimulator).

A built `idb_companion` distribution ships the shim dylibs in a `Resources` directory that sits alongside the `idb_companion` binary, and finds them there at runtime through [`BundledResources`](https://github.com/facebook/idb/blob/main/FBControlCore/Utility/BundledResources.swift). If you relocate the companion binary, move the whole distribution directory as a unit so that `Resources` stays next to it.
