...
@property (nonatomic, strong, nullable, readwrite) FBLaunchedProcess *process;
...
+ (FBFuture<id<SimulatorBridge>> *)bridgeForSimulator:(FBSimulator *)simulator
{
  ...
  return [[[self
    ...
    onQueue:simulator.workQueue map:^(NSArray<id> *tuple) {
      ...
      FBLaunchedProcess *process = (tuple.count == 2) ? tuple[1] : nil;
...
+ (FBFuture<FBLaunchedProcess * > *)bridgeOperationForSimulator:(FBSimulator *)simulator portName:(NSString *)portName
{
  NSError *error = nil;
  ...
  return [[[simulator
    ...
    onQueue:simulator.asyncQueue fmap:^(FBLaunchedProcess *process) {
      return [[[buffer
    ...
    onQueue:simulator.asyncQueue doOnResolved:^(FBLaunchedProcess *process) {
...
- (instancetype)initWithBridge:(NSProxy<SimulatorBridge> *)bridge process:(FBLaunchedProcess *)process workQueue:(dispatch_queue_t)workQueue asyncQueue:(dispatch_queue_t)asyncQueue
{
  self = [super init];
  if (!self) {
    return nil;
...
- (FBFuture<NSNull *> *)disconnect
{
  return [FBFuture
    onQueue:self.workQueue resolve:^ FBFuture<NSNull *> * {
      ...
      FBLaunchedProcess *process = self.process;
      self.process = nil;
      if (!process) {
        return FBFuture.empty;
      }
