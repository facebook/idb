...
@property (nonatomic, readwrite, assign) BOOL isRunningXcodeBuildOperation;
...
- (FBFutureContext<NSNumber *> *)transportForTestManagerService
{
  return [[[self
          ...
          onQueue:self.simulator.asyncQueue
          contextualTeardown:^(NSNumber *socketNumber, FBFutureState __) {
...
- (FBFuture<FBSubprocess *> *)_startTestWithLaunchConfiguration:(FBTestLaunchConfiguration *)configuration logger:(id<FBControlCoreLogger>)logger
{
  ...
  return [[FBXCTestShimConfiguration
           sharedShimConfigurationWithLogger:self.simulator.logger]
          onQueue:self.simulator.asyncQueue
          fmap:^(FBXCTestShimConfiguration *shims) {
            return [FBXcodeBuildOperation
