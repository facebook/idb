...
- (FBFuture<FBInstalledApplication *> *)installApplicationWithPath:(NSString *)path
{
  return [[self
          ...
          fmap:^FBFuture<FBInstalledApplication *> *(FBBundleDescriptor *appBundle) {
...
- (FBFuture<FBSimulatorLaunchedApplication *> *)launchApplication:(FBApplicationLaunchConfiguration *)configuration
{
  ...
  return [[[FBFuture futureWithFutures:@[
    ...
    [self confirmApplicationLaunchState:configuration.bundleID launchMode:configuration.launchMode waitForDebugger:configuration.waitForDebugger],
          ...
          onQueue:simulator.workQueue
          fmap:^FBFuture<FBSimulatorLaunchedApplication *> *(FBProcessFileAttachment *attachment) {
...
- (FBFuture<NSArray<FBInstalledApplication *> *> *)installedApplications
{
  return [[FBFuture
          ...
          map:^(NSDictionary<NSString *, id> *installedApps) {
...
- (FBFuture<FBInstalledApplication *> *)installedApplicationWithBundleID:(NSString *)bundleID
{
  return [FBFuture
          onQueue:self.simulator.workQueue
          resolveValue:^FBInstalledApplication *(NSError **error) {
...
- (FBFuture<NSDictionary<NSString *, NSNumber *> *> *)runningApplications
{
  ...
  return [[self.simulator
          ...
          onQueue:self.simulator.asyncQueue
          map:^(NSDictionary<NSString *, NSNumber *> *serviceNameToProcessIdentifier) {
...
- (FBInstalledApplication *)installedApplicationWithBundleID:(NSString *)bundleID error:(NSError **)error;
{
  ...
  if (!applicationIsInstalled) {
    return [[FBSimulatorError
             describe:[NSString stringWithFormat:@"Cannot get app information for '%@', it is not installed", bundleID]]
            fail:error];
  }
...
- (FBFuture<NSNumber *> *)confirmApplicationLaunchState:(NSString *)bundleID launchMode:(FBApplicationLaunchMode)launchMode waitForDebugger:(BOOL)waitForDebugger
{
  ...
  return [[simulator
          ...
          chain:^FBFuture<NSNull *> *(FBFuture<NSNumber *> *processFuture) {
...
- (FBFuture<NSNumber *> *)launchApplication:(FBApplicationLaunchConfiguration *)configuration stdOut:(id<FBProcessFileOutput>)stdOut stdErr:(id<FBProcessFileOutput>)stdErr
{
  ...
  return [[self
           ...
           stdOutPath:stdOut.filePath
           stdErrPath:stdErr.filePath]
          onQueue:self.simulator.workQueue
...
- (FBFuture<NSNumber *> *)isApplicationRunning:(NSString *)bundleID
{
  return [[self.simulator
           processIDWithBundleID:bundleID]
          onQueue:self.simulator.workQueue
          chain:^(FBFuture<NSNumber *> *future) {
...
- (FBFuture<NSNumber *> *)launchApplication:(FBApplicationLaunchConfiguration *)configuration stdOutPath:(NSString *)stdOutPath stdErrPath:(NSString *)stdErrPath
{
  ...
  [simulator.device launchApplicationAsyncWithID:configuration.bundleID
                                         options:options
                                 completionQueue:simulator.workQueue
                               completionHandler:^(NSError *error, pid_t pid) {
                                 if (error) {
                                   [logger log:[NSString stringWithFormat:@"Failed to launch Application %@ %@", configuration.bundleID, error]];
                                   [future resolveWithError:error];
                                 } else {
                                   [logger log:[NSString stringWithFormat:@"Launched Application %@ with pid %d", configuration.bundleID, pid]];
...
{
  NSMutableDictionary<NSString *, id> *options = [[FBSimulatorProcessSpawnCommands launchOptionsWithArguments:configuration.arguments
                                                                                                  environment:configuration.environment
                                                                                              waitForDebugger:configuration.waitForDebugger] mutableCopy];
  if (stdOutPath) {
...
+ (FBInstalledApplication *)installedApplicationFromInfo:(NSDictionary<NSString *, id> *)appInfo error:(NSError **)error
{
  ...
  if (![appName isKindOfClass:NSString.class]) {
    return [[FBControlCoreError
             describe:[NSString stringWithFormat:@"Bundle Name %@ is not a String for %@ in %@", appName, FBApplicationInstallInfoKeyBundleName, appInfo]]
            fail:error];
  }
  NSString *bundleIdentifier = appInfo[FBApplicationInstallInfoKeyBundleIdentifier];
  if (![bundleIdentifier isKindOfClass:NSString.class]) {
    return [[FBControlCoreError
             describe:[NSString stringWithFormat:@"Bundle Identifier %@ is not a String for %@ in %@", bundleIdentifier, FBApplicationInstallInfoKeyBundleIdentifier, appInfo]]
            fail:error];
  }
  NSString *appPath = appInfo[FBApplicationInstallInfoKeyPath];
  if (![appPath isKindOfClass:NSString.class]) {
    return [[FBControlCoreError
             describe:[NSString stringWithFormat:@"App Path %@ is not a String for %@ in %@", appPath, FBApplicationInstallInfoKeyPath, appInfo]]
            fail:error];
  }
  NSString *typeString = appInfo[FBApplicationInstallInfoKeyApplicationType];
  if (![typeString isKindOfClass:NSString.class]) {
    return [[FBControlCoreError
             describe:[NSString stringWithFormat:@"Install Type %@ is not a String for %@ in %@", typeString, FBApplicationInstallInfoKeyApplicationType, appInfo]]
            fail:error];
  }
  NSURL *dataContainer = appInfo[KeyDataContainer];
  if (dataContainer && ![dataContainer isKindOfClass:NSURL.class]) {
    return [[FBControlCoreError
             describe:[NSString stringWithFormat:@"Data Container %@ is not a NSURL for %@ in %@", dataContainer, KeyDataContainer, appInfo]]
            fail:error];
  }
