...
private struct AXBridgeReadPlan {
  ...
  init(
    ...
    self.attributes = AXWire.Node.fetchList(for: serializationKeys)
...
extension AXBridgeTreeReader {
  ...
  func describeTree(
    ...
    case let .marker(value, key, _, ignoresCase):
      ...
      guard
        let match = AXTreeWalk.matchingElement(
  ...
  private func refiningInteractable(
    ...
    element = AXScreenBoundsClassifier.notingScreenClipping(element, screen: screen)
    if options.keys.contains(.occludedBy),
      case let .blocked(reasons)?? = element.interactable,
      let found = element.explainedBy
    {
      let foundIdentity = AXElementIdentity(found)
