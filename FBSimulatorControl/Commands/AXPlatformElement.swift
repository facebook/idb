...
/// The read-only accessibility attribute and traversal surface. Actions live in `AXWritableElement`,
...
protocol AXPlatformElement: AnyObject {
  func axFrame() -> NSRect
  func axRole() -> String?
  func axLabel() -> String?
  func axValue() -> Any?
  ...
  func axExplainedBy() -> AXPlatformElement?
...
/// perform these, so a caller that acts on an element (`AccessibilityElement`) demands
...
extension AXPMacPlatformElement: AXPlatformElement {
