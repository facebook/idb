...
///
/// The serialize step (`AXTreeWalk.describeAllElements`) and the truncation warning happen
...
struct AXTreeRead: @unchecked Sendable {
  let tree: [String: Any]
  let pid: pid_t
  let truncated: Bool
...
struct AXReadTimings: Equatable {
...
extension AXTreeRead {
  ...
  static func guestPhases(fromResponse response: [String: Any]) -> (traverse: CFAbsoluteTime?, machRoundTrips: Int64?) {
    guard let phases = response[AXWire.Envelope.phases.rawValue] as? [String: Any] else {
      return (nil, nil)
    }
    // Milliseconds on the wire, seconds in the model, matching every other duration here.
    let traverse = (phases[AXWire.Phase.traverse.rawValue] as? Double).map { $0 / 1000 }
    let roundTrips = (phases[AXWire.Phase.machRoundTrips.rawValue] as? NSNumber)?.int64Value
