...
- Configuration values: `ApplicationLaunchConfiguration`, `SimulatorControlConfiguration`, `SimulatorConfiguration` & `SimulatorBootConfiguration`. The last three are structs.
...
Newer Xcode versions introduce a second event path: a `dtuhidd` daemon inside the Simulator, which receives events as XPC dictionaries rather than reverse-engineered mach structures. On these runtimes the guest moves some of its legacy HID services to `dtuhidd`, so `FBSimulatorControl` implements HID delivery behind a pluggable transport ([`SimulatorHID`](https://github.com/facebook/idb/blob/main/FBSimulatorControl/HID/SimulatorHID.swift)). The Indigo transport remains the default where it works, and the [DTUHID transport](https://github.com/facebook/idb/blob/main/FBSimulatorControl/HID/SimulatorDigitizerHIDTransport.swift) is used where the runtime requires it, negotiated automatically per Simulator. A caller of the Framework does not need to know which transport is in use.
