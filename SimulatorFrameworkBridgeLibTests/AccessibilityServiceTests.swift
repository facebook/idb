...
final class AccessibilityServiceTests: XCTestCase {
  ...
  func testInfiniteFrameValueSerializesAsNull() {
    let data = FBAccessibilityService.serializeResponse(Self.FBAXTestsFrameResponse(rect: CGRect(x: Double.infinity, y: 0, width: 10, height: 20)))
  ...
  func testFiniteResponseIsUnchanged() {
    let data = FBAccessibilityService.serializeResponse(Self.FBAXTestsFrameResponse(rect: CGRect(x: 16, y: 293, width: 370, height: 52)))
  ...
  func testUnserializableValueYieldsAnErrorFrameRatherThanRaising() {
    ...
    let parsed = Self.FBAXTestsParse(data: FBAccessibilityService.serializeResponse(response))
  ...
  func testNonStringVerbIsRejectedWithAnErrorFrame() {
    for verb in [123, ["a": 1], ["describe"], NSNull()] as [Any] {
      let response = FBAccessibilityService.handleRequest(["verb": verb])
  ...
  func testMissingVerbIsRejectedWithAnErrorFrame() {
    let response = FBAccessibilityService.handleRequest(["pid": 1234])
  ...
  func testGuestWireConstantsMatchTheHostContract() {
    ...
    XCTAssertEqual(FBAccessibilityService.wireConstantsForTesting(), expected)
  }
  ...
  func testModalDescriptorReportsSystemAlertWindowAsSystemKind() {
    ...
    let modal = FBAccessibilityService.modalDescriptor(tree)
  ...
  func testModalDescriptorReturnsNilWhenNoAlertPresent() {
    ...
    XCTAssertNil(FBAccessibilityService.modalDescriptor(tree))
  }
}
