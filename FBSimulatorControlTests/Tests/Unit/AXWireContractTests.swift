...
final class AXWireContractTests: XCTestCase {

  // MARK: - Node-attribute keys (the `_XCTD_fetchAttributes:` request/echo keys)

  func testNodeAttributeWireKeys() {
    XCTAssertEqual(AXWire.Node.explainedBy.rawValue, "FBExplainedBy")
    XCTAssertEqual(AXWire.Node.isEnabled.rawValue, "FBIsEnabled")
    XCTAssertEqual(AXWire.Node.translatorRole.rawValue, "FBTranslatorRole")
    XCTAssertEqual(AXWire.Node.translatorSubrole.rawValue, "FBTranslatorSubrole")
    XCTAssertEqual(AXWire.Node.traits.rawValue, "FBTraits")
    XCTAssertEqual(AXWire.Node.elementIdentity.rawValue, "FBElementIdentity")
    XCTAssertEqual(AXWire.Node.elementType.rawValue, "XC_kAXXCAttributeElementType")
    XCTAssertEqual(AXWire.Node.elementBaseType.rawValue, "XC_kAXXCAttributeElementBaseType")
    XCTAssertEqual(AXWire.Node.label.rawValue, "XC_kAXXCAttributeLabel")
    XCTAssertEqual(AXWire.Node.value.rawValue, "XC_kAXXCAttributeValue")
  ...
  func testFailureKindWireValues() {
    let expected: [AXWire.ErrorKind: String] = [
    ...
    for (kind, wireValue) in expected {
      ...
      XCTAssertEqual(AXWire.ErrorKind(rawValue: wireValue), kind, "\(kind) must round-trip through its wire value")
    }
  }
  ...
  func testAnUnknownFailureKindIsNotAKnownOne() {
    XCTAssertNil(AXWire.ErrorKind(rawValue: "some_future_kind"))
  }
  ...
  func testActionWireValues() {
    let expected: [AXWire.Action: String] = [
      .press: "press",
      .scrollUp: "scroll-up",
    ...
    XCTAssertEqual(Set(AXWire.Action.allCases), Set(expected.keys), "every action must have its wire value pinned")
  ...
  func testTheVerbContributesNoArgvPair() {
    XCTAssertEqual(AXWire.Request.verb.argument("describe"), [])
    XCTAssertEqual(AXWire.Request.maxNodes.argument("5000"), ["--max-nodes", "5000"])
  }
  ...
  func testAWriteRendersTheSameFieldsForBothTransports() {
    let request = AXBridgeWriteRequest(
      ...
      y: 406,
      pid: 4321,
      assertion: AXBridgeWriteAssertion(key: .label, value: "General")
    )
