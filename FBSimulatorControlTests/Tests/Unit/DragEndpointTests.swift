...
/// through `DragEndpoint` before resolving either, so this is the one place a drag decides which
...
final class DragEndpointTests: XCTestCase {
  ...
  func testACoordinateIsItsOwnEndpoint() throws {
    ...
    for backend in Self.backends {
      XCTAssertEqual(try DragEndpoint(.point(point), backend: backend), .point(point), "\(backend)")
    }
  }
