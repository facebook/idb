...
final class FBSimulatorGuestServicesSmokeTests: FBProvidedSimulatorTestCase {
  ...
  func testGuestServesLaunchCtlBridgeSettingsAndApplications() async throws {
    ...
    let original = try await simulator.getCurrentPreference("AutoFillPasswords", domain: nil)
    addTeardownBlock {
