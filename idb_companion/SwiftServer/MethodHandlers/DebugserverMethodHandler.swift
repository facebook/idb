  ...
  func handle(requestStream: GRPCAsyncRequestStream<Idb_DebugServerRequest>, responseStream: GRPCAsyncResponseStreamWriter<Idb_DebugServerResponse>, context: GRPCAsyncServerCallContext) async throws {
    ...
    for try await request in requestStream {
      ...
      case let .start(start):
        let debugServer = try await BridgeFuture.value(commandExecutor.debugserver_start(start.bundleID)) as! FBDebugServer
      ...
      case .status:
        ...
        if let debugServer = try? await BridgeFuture.value(commandExecutor.debugserver_status()) as? FBDebugServer {
          try await responseStream.send(debugserverStatusToProto(debugServer: debugServer))
        } else {
      ...
      case .stop:
        let debugServer = try await BridgeFuture.value(commandExecutor.debugserver_stop()) as! FBDebugServer
        try await responseStream.send(debugserverStatusToProto(debugServer: debugServer))
        return
