...
@property (nonatomic, readonly, strong) FBIDBLogger *logger;
...
{
  return [self installExtractedApp:[self.temporaryDirectory withArchiveExtractedFromStream:input compression:compression overrideModificationTime:overrideModificationTime]
...
{
  return [self installAndLinkDsym:[self dsymDirnameFromUnzipDir:[self.temporaryDirectory withArchiveExtractedFromStream:input compression:compression]]
                      intoStorage:self.storageManager.dsym
                           linkTo:linkTo];
}
...
- (FBFuture<NSData *> *)take_screenshot:(FBScreenshotFormat)format
{
  return [[self
           screenshotCommands]
          onQueue:self.target.workQueue
          fmap:^(id<FBScreenshotCommands> commands) {
...
- (FBFuture<FBAccessibilityElementsResponse *> *)accessibility_info_at_point:(nullable NSValue *)value nestedFormat:(BOOL)nestedFormat
{
  return [[self
          ...
          onQueue:self.target.workQueue
          fmap:^FBFuture *(id<FBAccessibilityCommands> commands) {
            ...
            return [elementFuture
                    onQueue:self.target.workQueue
                    map:^FBAccessibilityElementsResponse *(FBAccessibilityElement *element) {
                      NSError *error = nil;
...
- (FBFuture<NSNull *> *)add_media:(NSArray<NSURL *> *)filePaths
{
  return [self.mediaCommands
          ...
          fmap:^FBFuture *(id<FBSimulatorMediaCommands> commands) {
...
- (FBFuture<NSNull *> *)approve:(NSSet<FBTargetSettingsService> *)services for_application:(NSString *)bundleID
{
  return [self.settingsCommands
          onQueue:self.target.workQueue
          fmap:^FBFuture *(id<FBSimulatorSettingsCommands> commands) {
...
{
  return [[self list_apps:NO] onQueue:self.target.workQueue
                                 fmap:^FBFuture<NSNull *> *(NSDictionary<FBInstalledApplication *, id> *apps) {
                                   ...
                                   for (FBInstalledApplication *app in apps) {
                                     if (app.installType == FBApplicationInstallTypeUser) {
                                       [uninstall_futures addObject:[[self kill_application:app.bundle.identifier] onQueue:self.target.workQueue
...
- (FBFuture<NSNull *> *)clean
{
  ...
  return [[self uninstall_all_applications] onQueue:self.target.workQueue
                                               fmap:^FBFuture<NSNull *> *(id _) {
                                                 return [self remove_all_storage_and_clear_keychain];
                                               }];
}
...
- (FBFuture<NSNull *> *)update_contacts:(NSData *)dbTarData
{
  return [[self.temporaryDirectory
          ...
          onQueue:self.target.workQueue
          pop:^(NSURL *tempDirectory) {
            return [self.settingsCommands onQueue:self.target.workQueue
                                             fmap:^FBFuture *(id<FBSimulatorSettingsCommands> commands) {
...
{
  return [FBFuture onQueue:self.target.workQueue
                   resolve:^{
                     NSError *error;
...
- (FBFuture<NSArray<NSString *> *> *)list_tests_in_bundle:(NSString *)bundleID with_app:(NSString *)appPath
{
  ...
  return [FBFuture
          onQueue:self.target.workQueue
          resolve:^FBFuture<NSArray<NSString *> *> * {
            NSError *error = nil;
...
- (FBFuture<id<FBDebugServer>> *)debugserver_stop
{
  return [[[self
            debugserver_status]
           onQueue:self.target.workQueue
           fmap:^(id<FBDebugServer> debugServer) {
...
- (FBFuture<NSArray<FBCrashLogInfo *> *> *)crash_list:(NSPredicate *)predicate
{
  return [[self.target
           ...
           useCache:NO]
          onQueue:self.target.asyncQueue
          map:^(NSArray<FBCrashLogInfo *> *crashes) {
            return crashes;
          }];
}
...
- (FBFuture<NSNull *> *)hid:(id<FBSimulatorHIDEventProtocol>)event
{
  return [self.connectToHID
          onQueue:self.target.workQueue
          fmap:^FBFuture *(FBSimulatorHID *hid) {
...
- (FBFuture<NSNull *> *)set_hardware_keyboard_enabled:(BOOL)enabled
{
  return [[self
           settingsCommands]
          onQueue:self.target.workQueue
          fmap:^(id<FBSimulatorSettingsCommands> commands) {
...
- (FBFuture<NSNull *> *)push_file_from_tar:(NSData *)tarData to_path:(NSString *)destinationPath containerType:(NSString *)containerType
{
  return [[self.temporaryDirectory
          ...
          onQueue:self.target.workQueue
          pop:^FBFuture *(NSURL *extractionDirectory) {
            NSError *error;
...
- (FBFuture<NSDictionary<NSString *, NSArray<NSString *> *> *> *)list_paths:(NSArray<NSString *> *)paths containerType:(NSString *)containerType
{
  return [[[self
          ...
          onQueue:self.target.asyncQueue
          map:^(NSArray<NSArray<NSString *> *> *listings) {
...
- (FBFuture<FBInstalledArtifact *> *)installAppBundle:(FBFutureContext<FBBundleDescriptor *> *)bundleContext makeDebuggable:(BOOL)makeDebuggable
{
  ...
  return [bundleContext
          onQueue:self.target.asyncQueue
          pop:^(FBBundleDescriptor *appBundle) {
            if (!appBundle) {
...
- (FBFuture<FBInstalledArtifact *> *)installAndLinkDsym:(FBFutureContext<NSURL *> *)extractedFileContext intoStorage:(FBFileStorage *)storage linkTo:(nullable FBDsymInstallLinkToBundle *)linkTo
{
  return [extractedFileContext
          ...
          pop:^(NSURL *extractionDir) {
            ...
            return [future onQueue:self.target.workQueue
                              fmap:^(NSURL *bundlePath) {
...
- (FBFuture<FBInstalledArtifact *> *)installBundle:(FBFutureContext<NSURL *> *)extractedDirectoryContext intoStorage:(FBBundleStorage *)storage
{
  return [extractedDirectoryContext
          onQueue:self.target.workQueue
          pop:^FBFuture<FBInstalledArtifact *> *(NSURL *extractedDirectory) {
            NSError *error = nil;
