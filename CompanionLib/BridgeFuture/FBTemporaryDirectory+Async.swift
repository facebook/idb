...
public extension FBTemporaryDirectory {
  ...
  /// Extracts the archive from `input` into a temporary directory and returns its URL. The directory's
  /// teardown is registered with the current `FBTeardownContext`.
  ...
  /// Extracts the archive from `input` into a temporary directory, then returns the unique file inside
  /// each immediate subdirectory. The directory's teardown is registered with the current
  /// `FBTeardownContext`.
