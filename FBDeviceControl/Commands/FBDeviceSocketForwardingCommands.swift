...
public class FBDeviceSocketForwardingCommands: NSObject, FBSocketForwardingCommands {
  ...
  public func drainLocalFileInput(
    ...
    return consumer(forRemotePort: Int(remotePort), writingTo: localConsumer).onQueue(device!.asyncQueue, pop: { remoteConsumer -> FBFuture<AnyObject> in
      ...
      return reader.startReading().onQueue(self.device!.asyncQueue, map: { _ in
  ...
  private func consumer(forRemotePort remotePort: Int, writingTo consumer: any FBDataConsumer) -> FBFutureContext<any FBDataConsumer> {
    localSocketFromRemotePort(remotePort: remotePort).onQueue(device!.asyncQueue, pend: { (remoteSocket: NSNumber) -> FBFuture<AnyObject> in
      var error: NSError?
  ...
  private func localSocketFromRemotePort(remotePort: Int) -> FBFutureContext<NSNumber> {
    ...
    return device!.connectToDevice(withPurpose: "Socket Connection").onQueue(device!.workQueue, pend: { (device: any FBDeviceCommands) -> FBFuture<AnyObject> in
    ...
    }).onQueue(device!.asyncQueue, contextualTeardown: { (localSocketNumber, _) -> FBFuture<NSNull> in
