...
@property (nonatomic, readonly, strong) FBAFCConnection *connection;
...
- (FBFuture<NSString *> *)copyFromContainer:(NSString *)sourcePath toHost:(NSString *)destinationPath
{
  return [[self.springboard
          ...
          onQueue:self.queue
          fmap:^FBFuture<NSString *> *(NSData *data) {
            NSError *error = nil;
...
- (FBFuture<NSArray<NSString *> *> *)contentsOfDirectory:(NSString *)path
{
  return [[self
           allDiskImagePaths]
          onQueue:self.queue
          fmap:^(NSArray<NSString *> *diskImagePaths) {
            NSError *error = nil;
...
- (FBFuture<NSDictionary<NSString *, FBDeveloperDiskImage *> *> *)mountedDiskImages
{
  return [[self.commands
           mountedDiskImages]
          onQueue:self.queue
          map:^(NSArray<FBDeveloperDiskImage *> *mountedImages) {
...
- (FBFuture<NSArray<NSString *> *> *)allDiskImagePaths
{
  return [[self
           mountedDiskImages]
          onQueue:self.queue
          map:^(NSDictionary<NSString *, FBDeveloperDiskImage *> *mountedDiskImages) {
...
- (FBFuture<NSArray<NSString *> *> *)contentsOfDirectory:(NSString *)path
{
  return [[self.commands
           listSymbols]
          onQueue:self.queue
          map:^(NSArray<NSString *> *listedSymbols) {
...
@property (nonatomic, readonly, strong) FBDevice *device;
@property (nonatomic, readonly, assign) AFCCalls afcCalls;
