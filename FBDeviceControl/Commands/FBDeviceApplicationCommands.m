...
@property (nullable, nonatomic, readwrite, copy) NSDictionary<NSString *, id> *lastEvent;
...
@property (nonatomic, readonly, copy) NSURL *deltaUpdateDirectory;
...
@property (nonatomic, readonly, strong) FBDeviceApplicationCommands *commands;
@property (nonatomic, readonly, strong) dispatch_queue_t queue;
@property (nonatomic, readonly, strong) FBApplicationLaunchConfiguration *configuration;
...
- (FBFuture<NSArray<FBInstalledApplication *> *> *)installedApplications
{
  return [[self
          ...
          onQueue:self.device.asyncQueue
          map:^(NSDictionary<NSString *, NSDictionary<NSString *, id> *> *applicationData) {
...
- (FBFuture<FBInstalledApplication *> *)installedApplicationWithBundleID:(NSString *)bundleID
{
  return [[self
           installedApplicationsData:FBDeviceApplicationCommands.installedApplicationLookupAttributes]
          onQueue:self.device.asyncQueue
          fmap:^FBFuture *(NSDictionary<NSString *, NSDictionary<NSString *, id> *> *applicationData) {
...
- (FBFuture<NSDictionary<NSString *, NSNumber *> *> *)runningApplications
{
  return [[FBFuture
          ...
          onQueue:self.device.asyncQueue
          map:^NSDictionary<NSString *, NSNumber *> *(NSArray<id> *tuple) {
...
- (FBFuture<NSNumber *> *)processIDWithBundleID:(NSString *)bundleID
{
  return [[self
           runningApplications]
          onQueue:self.device.asyncQueue
          fmap:^(NSDictionary<NSString *, NSNumber *> *result) {
...
- (FBFuture<NSNull *> *)killApplicationWithBundleID:(NSString *)bundleID
{
  return [[self
          ...
          onQueue:self.device.workQueue
          fmap:^(NSNumber *processIdentifier) {
...
- (FBFuture<id<FBLaunchedApplication>> *)launchApplication:(FBApplicationLaunchConfiguration *)configuration
{
  if (self.device.osVersion.version.majorVersion >= 17) {
    ...
    return [[devicectl launchApplicationWithConfiguration:configuration]
            onQueue:self.device.asyncQueue
            map:^id<FBLaunchedApplication>(NSNumber *pid) {
    ...
    return [[[self
             ...
             onQueue:self.device.asyncQueue
             pop:^(FBInstrumentsClient *client) {
