...
public class FBDeviceApplicationCommands: NSObject, FBApplicationCommands {
  ...
  @objc public func installApplication(withPath path: String) -> FBFuture<FBInstalledApplication> {
    ...
    return device!.connectToDevice(withPurpose: "install").onQueue(device!.workQueue, pop: { (d: AnyObject) -> FBFuture<AnyObject> in
      ...
      let status = device.calls.SecureInstallApplicationBundle?(
        device.amDeviceRef,
        appURL as CFURL,
    ...
    }).onQueue(device!.asyncQueue, fmap: { (_: AnyObject) -> FBFuture<AnyObject> in
  ...
  @objc public func uninstallApplication(withBundleID bundleID: String) -> FBFuture<NSNull> {
    return device!.connectToDevice(withPurpose: "uninstall_\(bundleID)").onQueue(device!.workQueue, pop: { (d: AnyObject) -> FBFuture<AnyObject> in
      let device = d as! any FBDeviceCommands
      ...
      let context = Unmanaged.passUnretained(statistics).toOpaque()
      let status = device.calls.SecureUninstallApplication?(
        nil,
        device.amDeviceRef,
  ...
  @objc public func installedApplications() -> FBFuture<NSArray> {
    return installedApplicationsData(Self.installedApplicationLookupAttributes).onQueue(device!.asyncQueue, map: { (data: AnyObject) -> AnyObject in
  ...
  @objc public func installedApplication(withBundleID bundleID: String) -> FBFuture<FBInstalledApplication> {
    return installedApplicationsData(Self.installedApplicationLookupAttributes).onQueue(device!.asyncQueue, fmap: { (data: AnyObject) -> FBFuture<AnyObject> in
      let applicationData = data as! [String: [String: Any]]
  ...
  @objc public func runningApplications() -> FBFuture<NSDictionary> {
    return pidToRunningProcessName().onQueue(device!.asyncQueue, fmap: { (pidData: AnyObject) -> FBFuture<AnyObject> in
      let pidToRunningProcessName = pidData as! [NSNumber: String]
      return self.installedApplicationsData(Self.namingLookupAttributes).onQueue(self.device!.asyncQueue, map: { (appData: AnyObject) -> AnyObject in
  ...
  @objc public func processID(withBundleID bundleID: String) -> FBFuture<NSNumber> {
    return runningApplications().onQueue(device!.asyncQueue, fmap: { (result: AnyObject) -> FBFuture<AnyObject> in
  ...
  @objc public func killApplication(withBundleID bundleID: String) -> FBFuture<NSNull> {
    return processID(withBundleID: bundleID).onQueue(device!.workQueue, fmap: { (pid: AnyObject) -> FBFuture<AnyObject> in
  ...
  @objc public func launchApplication(_ configuration: FBApplicationLaunchConfiguration) -> FBFuture<any FBLaunchedApplication> {
    if device!.osVersion.version.majorVersion >= 17 {
      ...
      return devicectl.launchApplication(configuration: configuration).onQueue(device!.asyncQueue, map: { (pid: AnyObject) -> AnyObject in
      ...
      return remoteInstrumentsClient().onQueue(device!.asyncQueue, pop: { (client: AnyObject) -> FBFuture<AnyObject> in
      ...
      }).onQueue(device!.asyncQueue, map: { (pid: AnyObject) -> AnyObject in
        let pidNumber = pid as! NSNumber
        return FBDeviceLaunchedApplication(
          processIdentifier: pidNumber.int32Value,
          configuration: configuration,
  ...
  private func installedApplicationsData(_ returnAttributes: [String]) -> FBFuture<NSDictionary> {
    return device!.connectToDevice(withPurpose: "installed_apps").onQueue(device!.workQueue, pop: { (d: AnyObject) -> FBFuture<AnyObject> in
      let device = d as! any FBDeviceCommands
      let options: [String: Any] = [
        "ReturnAttributes": returnAttributes,
      ]
      var applications = Unmanaged<CFDictionary>.passUnretained(NSDictionary() as CFDictionary)
      let status = device.calls.LookupApplications?(
        device.amDeviceRef,
        options as CFDictionary,
        &applications
      ) ?? -1
  ...
  private func remoteInstrumentsClient() -> FBFutureContext<FBInstrumentsClient> {
    ...
    return unsafeBitCast(
      device!.ensureDeveloperDiskImageIsMounted()
        .onQueue(device!.workQueue, pushTeardown: { (_: AnyObject) -> FBFutureContext<AnyObject> in
  ...
  private func pidToRunningProcessName() -> FBFuture<NSDictionary> {
    return device!.startService("com.apple.os_trace_relay").onQueue(device!.asyncQueue, pop: { (connection: AnyObject) -> FBFuture<AnyObject> in
      let conn = connection as! FBAMDServiceConnection
      do {
      ...
      for (processIdentifier, value) in payload {
        guard let contents = value as? [String: Any],
              let processName = contents["ProcessName"] as? String else {
          continue
        }
