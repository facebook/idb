...
- (FBFuture<NSNull *> *)runTestWithLaunchConfiguration:(FBTestLaunchConfiguration *)testLaunchConfiguration reporter:(id<FBXCTestReporter>)reporter logger:(id<FBControlCoreLogger>)logger
{
  ...
  return [[[[FBXcodeBuildOperation
           ...
           onQueue:self.device.workQueue
           fmap:^(FBSubprocess<id, id, id> *task) {
...
- (FBFuture<FBSubprocess<id, id, id> *> *)_startTestWithLaunchConfiguration:(FBTestLaunchConfiguration *)configuration logger:(id<FBControlCoreLogger>)logger
{
  NSError *error = nil;
